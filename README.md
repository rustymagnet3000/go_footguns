...
| 03 | [map dedup & iteration order](./03-map-dedup-order) | A map dedups a slice in O(n) (vs the nested-loop O(n²) trap), but ranging the map for the result gives a different order every run — map iteration is randomized by design. Keep a slice for order. |
| 04 | [ignored errors fail *open*](./04-error-fail-open) | A security check whose only failure signal is an `error` silently *passes* when that error is discarded. Go lets you drop it as a bare statement; exception-based languages would crash instead. |
| 05 | [concurrent map write = fatal crash](./05-concurrent-map-panic) | Racing writes to a built-in `map` trigger `fatal error: concurrent map writes` — an *unrecoverable* runtime crash (`recover()` can't catch it), so one racy access is a whole-process DoS. Java/Python don't kill the runtime. |
